...
local CARD_TOP_MARGIN            = 130 -- gap between "Pick a card!" hint and the cards
...
-- ── Sound helper: clones a static source so overlapping plays
--    don't cut each other off (e.g. rapid clicks, many sparkles) ─
local function PlaySfx(src)
  if not src then return end
  love.audio.play(src:clone())
end
...
local function SpawnFirework(x, y)
  ...
  PlaySfx(sounds.firework) -- "firework boom + crackle"
end
...
function love.load()
  ...
  sounds        = {
    ...
    card_ding   = love.audio.newSource("sounds/card_ding.mp3", "static"),
  ...
  shop          = { x = 0, y = window.height, width = window.width, height = window.height * 0.82 }
...
function love.update(dt)
  ...
  if next_milestone_idx <= #milestones and score >= milestones[next_milestone_idx] then
    ...
    PlaySfx(sounds.milestone) -- triumphant milestone fanfare
  end
...
function UpdateCardAnim(dt)
  ...
  if anim_state == "to_center" then
    ...
    if math.abs(card.ax - cx) < 2 and math.abs(card.ay - cy) < 2 then
      ...
      PlaySfx(sounds.card_whoosh) -- "card whoosh"
    end
  ...
  elseif anim_state == "show_result" then
    if anim_t >= DUR_SHOW then
      ...
      PlaySfx(sounds.card_swoosh) -- "swoosh travel"
    end
  elseif anim_state == "travel" then
    ...
    if anim_t >= DUR_TRAVEL then
      ...
      ts.pumped = true
      PlaySfx(sounds.card_merge) -- "merge / impact boom"
    end
...
function PositionCards()
  local top_margin = CARD_TOP_MARGIN
...
function OpenShop()
  ...
  InitCards()
  PositionCards()
  PlaySfx(sounds.shop_open) -- "shop curtain rise / fanfare"
end
...
function love.mousepressed(mx, my, button)
  ...
  if not shop_displayed and anim_state == "idle" then
    if mx > cookie.left and mx < cookie.right
      ...
      PlaySfx(sounds.click) -- "cookie crunch / click"
    end
  end
  ...
  if shop_displayed and can_click_card and anim_state == "idle" then
    for _, card in ipairs(cards) do
      if card.visible
        ...
        PlaySfx(sounds.card_pick) -- "card pick / swipe"
        break
      end
    end
  end
